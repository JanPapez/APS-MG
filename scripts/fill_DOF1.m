function fill_DOF1(meshdata,m,j)
...
DOF1(j).BC = BC;
DOF1(j).dirichlet = find(BC==-1);
DOF1(j).freenodes = setdiff(1:lastFEMindex,DOF1(j).dirichlet);

% number of basis functions to data structure DOF(j)
DOF1(j).m = m;
DOF1(j).nFEM = nFEM;
DOF1(j).lastFEMindex = lastFEMindex;

DOF1(j).elementFEM = elementFEM;

DOF1(j).dirichletvalues = sparse(dirichletvalues(1:lastFEMindex));

DOF1(j).freenodesnumbering_toglobal = zeros(lastFEMindex,1);
temp = cumsum(DOF1(j).BC);
DOF1(j).freenodesnumbering_toglobal(DOF1(j).freenodes) = DOF1(j).freenodes(:) + temp(DOF1(j).freenodes);
