function [elements_sorted] = find_patch_elements_counterclockwise(j)
% function for searching the elements of each patch and orienting them
% anti-clockwise
%
% Jan Papez, Ani Miraci, December 2022
%       APS-MG MATLAB package https://github.com/JanPapez/APS-MG

global meshdata DOF2;

elements_sorted = struct([]);

% find neighbours of the elements
neighbours = all_neighbours( meshdata(j).elements );

% conversion tables, to set the "next" element in (anti)clocwise direction
conv_clockwise = [3 1 2];
conv_counterclockwise = [2 3 1];

for index = 1:meshdata(j).nc
    
    [~,elements_of_patch] = find(meshdata(j).elements(1:3,:) == index);
    nelements_of_patch = length(elements_of_patch);
    
    elements_sorted{index} = zeros(nelements_of_patch,1);
    
    if DOF2(j).BC(index) > -1
        % the vertex is inside the domain, we can start numbering anywhere
        element = elements_of_patch(1);
        for elem_index = 1:nelements_of_patch
            elements_sorted{index}(elem_index) = element;
            element = neighbours(conv_counterclockwise(meshdata(j).elements(1:3,element) == index),element);
        end
    else % the vertex is on the boundary
        for elem_index = 1:nelements_of_patch
            neigh_element = neighbours(conv_clockwise(meshdata(j).elements(1:3,elements_of_patch(elem_index)) == index),elements_of_patch(elem_index));
            if neigh_element == -1
                to_start = elem_index;
                break
            end
        ...
        %
        element = elements_of_patch(to_start);
        for elem_index = 1:nelements_of_patch
            elements_sorted{index}(elem_index) = element;
            element = neighbours(conv_counterclockwise(meshdata(j).elements(1:3,element) == index),element);
        end
