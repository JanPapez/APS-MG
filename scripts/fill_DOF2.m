function fill_DOF2(meshdata,m,j)
% function for indexing the degrees of freedom
% using John Burkardt's TRIANGULATION_TRIANGLE_NEIGHBORS code
%
% Jan Papez, Ani Miraci, December 2022
%       APS-MG MATLAB package https://github.com/JanPapez/APS-MG

global solution DOF2;

% dirichlet = unique(meshdata(j).edges(1:2,:));
boundary = meshdata(j).edges(:,max(meshdata(j).edges(6,:)==0, meshdata(j).edges(7,:)==0));
dirichlet = unique(boundary(1:2,:));

% number of basis functions
nFEM = (m+1)*(m+2)/2;

% first empty index
lastFEMindex = meshdata(j).nc;	% FEM basis functions

% indices of the basis function coresponding to vertices of triangulation        
verticesindices = [1; nFEM; m+1];

edgeindices = zeros(3,m+1);
% index of the edge is equal to the index of the oposite vertex
edgeindices(3,:) = (nFEM+1) - cumsum(1:m+1);
edgeindices(1,:) = cumsum(m+1:-1:1);
edgeindices(2,:) = 1:m+1;

alledgeindices = unique([1:m,cumsum(m+1:-1:1),(nFEM+1) - cumsum(1:m+1)]);
innerindices = setdiff(1:nFEM,alledgeindices);
innerindicescount = length(innerindices);

% find neighbours of the elements
neighbours = all_neighbours( meshdata(j).elements );

% element matrix containing the indices of all degrees of freedom
elementFEM = zeros(nFEM,meshdata(j).ne);
elementFEM(verticesindices,:) = meshdata(j).elements((1:3),:);

% field of boundary conditions for nodes; -1 stands for Dir.BC
BC = zeros(meshdata(j).nc*nFEM*nFEM,1);
BC(dirichlet) = -1;
    % is longer than needed, will be cut at the end of script

dirichletvalues = zeros(meshdata(j).nc*nFEM*nFEM,1);    
dirichletvalues(dirichlet) = ...
    solution.u(meshdata(j).coord(1,dirichlet),meshdata(j).coord(2,dirichlet));

othernodes = [3 1 2;...
              2 3 1];
          
nonlagrangean_basis = nonlagrangeannodes(m);
nonuniform_spacing_on_boundary = nonlagrangean_basis(2:m,2)';
          
for Tindex = 1:meshdata(j).ne
    coordinates = meshdata(j).coord(:,meshdata(j).elements(1:3,Tindex));
    
    elementFEM(innerindices,Tindex) = ...
        (lastFEMindex+1):(lastFEMindex+innerindicescount);
    lastFEMindex = lastFEMindex+innerindicescount;
    
    for ie = 1:3
        
        hatTindex = neighbours(ie,Tindex);
        
        if (hatTindex == -1) % hatTindex = -1 => tindex is on the boundary
            elementFEM(edgeindices(ie,2:m),Tindex) = (lastFEMindex+1):(lastFEMindex+m-1);
            BC((lastFEMindex+1):(lastFEMindex+m-1)) = -1;
            
            node1 = coordinates(:,othernodes(1,ie));
            node2 = coordinates(:,othernodes(2,ie));
            %edgenodes = (node2*(1:(m-1)) + node1*((m-1):-1:1))/m;
            edgenodes = node2*nonuniform_spacing_on_boundary + node1*nonuniform_spacing_on_boundary(end:-1:1);
            
            dirichletvalues((lastFEMindex+1):(lastFEMindex+m-1)) = ...
                solution.u(edgenodes(1,:),edgenodes(2,:));
            
            lastFEMindex = lastFEMindex + m-1;
        
        elseif (hatTindex < Tindex) % edge DOF(j)s are already numbered

            % finding the common edge of T and hatT       
            ihat = find(neighbours(:,hatTindex) == Tindex);

            elementFEM(edgeindices(ie,m:-1:2),Tindex) = ...
                elementFEM(edgeindices(ihat,2:m),hatTindex);
            
        else % edge DOF(j)s are not yet numbered
            elementFEM(edgeindices(ie,2:m),Tindex) = ...
                (lastFEMindex+1):(lastFEMindex+m-1);
            lastFEMindex = lastFEMindex + m-1;        
        end
...
BC = BC(1:lastFEMindex);

% dirichlet boundary condition
DOF2(j).BC = BC;
DOF2(j).dirichlet = find(BC==-1);
DOF2(j).freenodes = setdiff(1:lastFEMindex,DOF2(j).dirichlet);

% number of basis functions to data structure DOF(j)
DOF2(j).m = m;
DOF2(j).nFEM = nFEM;
DOF2(j).lastFEMindex = lastFEMindex;
DOF2(j).elementFEM = elementFEM;

DOF2(j).dirichletvalues = sparse(dirichletvalues(1:lastFEMindex));

DOF2(j).freenodesnumbering_toglobal = zeros(lastFEMindex,1);
% a faster variant below
% for k = 1:length(DOF(j).freenodes)
%     tmp = DOF(j).freenodes(k);
%     DOF(j).freenodesnumbering_toglobal(tmp) = tmp + sum(DOF(j).BC(1:tmp));
% end
temp = cumsum(DOF2(j).BC);
DOF2(j).freenodesnumbering_toglobal(DOF2(j).freenodes) = DOF2(j).freenodes(:) + temp(DOF2(j).freenodes);
