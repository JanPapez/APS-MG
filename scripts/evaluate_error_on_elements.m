...
X = ref_element(j).level.quadrature.X;
Y = ref_element(j).level.quadrature.Y;
W = ref_element(j).level.quadrature.Weights;
s = ref_element(j).level.quadrature.s;
...
for ie = 1:meshdata(j).ne
    ...
    locFEMgrad = ref_element(j).level.FEMgrad/Jloc;
